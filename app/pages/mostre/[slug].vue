...
const { data, pending, error } = await useAsyncData(
  () => `exhibition-detail-${slug.value}-${currentLocale.value}`,
  () =>
...
function getExhibitionTitle(item: any) {
  return item?.title ||
    item?.name ||
...
function getDateLabel(item: any) {
  const start =
    item?.start_date ||
    item?.startDate ||
    item?.date_start ||
...
const description = computed(() => {
  return getText(
    exhibition.value?.description ||
    exhibition.value?.descrizione ||
    exhibition.value?.content ||
    exhibition.value?.body
  )
})

const artists = computed(() => {
  return getRelationItems(exhibition.value?.artists)
})

const artworks = computed(() => {
  return getRelationItems(exhibition.value?.artworks)
})

const projects = computed(() => {
  return getRelationItems(exhibition.value?.projects)
})

const dateLabel = computed(() => {
  return getDateLabel(exhibition.value)
})

useSeoMeta({
  title: () =>
    exhibition.value
      ? `${getExhibitionTitle(exhibition.value)} | Thetis Gallery`
      : 'Mostra | Thetis Gallery',

  description: () =>
    description.value
      ? description.value.slice(0, 160)
      : 'Scheda mostra della galleria.'
})
</script>
...
<template>
  <main class="page">
    ...
    <section v-else-if="!exhibition" class="state">
      <h1>Mostra non trovata</h1>

      <NuxtLink to="/mostre" class="back-link">
        Torna alle mostre
      </NuxtLink>
    </section>
    ...
    <template v-else>
      <section class="exhibition-hero">
        ...
        <div class="exhibition-info">
          ...
          <p class="eyebrow">Mostra</p>

          <h1>{{ getExhibitionTitle(exhibition) }}</h1>
      ...
      <section v-if="projects.length" class="content-section">
        <div class="section-heading">
